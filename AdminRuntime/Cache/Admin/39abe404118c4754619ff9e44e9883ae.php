            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><a href="<?php echo U('Vpnaddress/addVpnAddress');?>"><button type="button" class="btn btn-block btn-primary">新增vpn地址</button></a></h3>
                         <h3 class="box-title"><a href="javascript:void(0)')}"><button type="button" class="btn btn-block btn-primary" onclick="delAll('<?php echo U('Vpnaddress/deleteaddrs');?>')">批量删除</button></a></h3>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            <tbody>
                            <?php if(is_array($allAccounts)): $i = 0; $__LIST__ = $allAccounts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tnd" style = "text-align: center;">
                                    <td><input type="checkbox" value="<?php echo ($vo["address_id"]); ?>" name="delAll"></td>
                                    <td><?php echo ($vo["address_id"]); ?></td>
                                    <td><?php echo ($vo["address_addr"]); ?>,<?php echo ($vo["address_city"]); ?></td>
                                     <td><?php echo ($vo["address_url"]); ?></td>
                                    <td>
                                        <a href="<?php echo U('Vpnaddress/editvpnaddress',array('address_id'=>$vo['address_id'],'address_addr'=>$vo['address_addr'],'address_city'=>$vo['address_city']));?>">编辑</a>
                                        <a href="javascript:if(confirm('确实要删除吗?'))location='<?php echo U('Vpnaddress/deletevpnaddress',array('address_id'=>$vo['address_id']));?>'">删除</a>
                                    </td>
