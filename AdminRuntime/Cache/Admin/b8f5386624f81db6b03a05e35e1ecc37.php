            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><a href="<?php echo U('VpnAccount/addVpnAccount');?>"><button type="button" class="btn btn-block btn-primary">新增vpn账号</button></a></h3>
                         <h3 class="box-title"><a href="javascript:void(0)')}"><button type="button" class="btn btn-block btn-primary" onclick="delAll('<?php echo U('VpnAccount/deleteaccounts');?>')">批量删除</button></a></h3>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            <?php if(is_array($allAccounts)): $i = 0; $__LIST__ = $allAccounts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tnd" style = "text-align: center;">
                                    <td><input type="checkbox" value="<?php echo ($vo["account_id"]); ?>" name="delAll"></td>
                                    <td><?php echo ($vo["account_id"]); ?></td>
                                    <td><?php echo ($vo["equipment"]); ?></td>
                                    <td><?php echo ($vo["account_name"]); ?></td>
                                    <td>
                                        <a href="<?php echo U('VpnAccount/editvpnaccount',array('account_id'=>$vo['account_id']));?>">编辑</a>
                                        <a href="javascript:if(confirm('确实要删除吗?'))location='<?php echo U('VpnAccount/deletevpnaccount',array('account_id'=>$vo['account_id']));?>'">删除</a>
                                    </td>
