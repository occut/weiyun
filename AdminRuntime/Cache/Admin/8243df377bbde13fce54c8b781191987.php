        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                   ...
                   <h3 class="box-title"><a href="#"><button type="button" class="btn btn-block btn-primary">分配权限</button></a></h3>
                 
                    </div>
                    <!-- /.box-header -->
            <!-- /.box-header -->
            <!-- form start -->
                <form action="<?php echo U('Admin/addAdminAndRoles');?>" method="post" >
                        <input type="hidden" name="adminid" value="<?php echo ($admin["admin_id"]); ?>" >
                        <div class="box-body">
                            <div class="form-group">
                               <label for="adminname">用户名：</label>
                               <span id="font1"><?php echo ($admin["admin_name"]); ?></span>
                            </div>

                         <!-- checkbox -->
              <div class="form-group">
                  
                  <?php if(is_array($roles)): $i = 0; $__LIST__ = $roles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="look">
                    <input type="checkbox" class="flat-red" name="roleid[]" value="<?php echo ($vo["role_id"]); ?>"  
                     <?php if(is_array($adminrole)): $i = 0; $__LIST__ = $adminrole;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i; if(($vo["role_id"]) == $voo["role_id"]): ?>checked="checked"<?php endif; endforeach; endif; else: echo "" ;endif; ?>
                           >
